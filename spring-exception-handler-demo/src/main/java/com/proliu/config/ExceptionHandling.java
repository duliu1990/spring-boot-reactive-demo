...
import com.proliu.dto.FieldErrorVM;
...
public class ExceptionHandling implements ProblemHandling, SecurityAdviceTrait {
    ...
    public static final String PROBLEM_BASE_URL = "https://www.manulife.com.hk";
    public static final URI DEFAULT_TYPE = URI.create(PROBLEM_BASE_URL);
    public static final URI CONSTRAINT_VIOLATION_TYPE = URI.create(PROBLEM_BASE_URL + "/constraint-violation");

    private final Environment env;

    public ExceptionHandling(Environment env) {
        this.env = env;
    }
    ...
    public Mono<ResponseEntity<Problem>> process(@Nullable ResponseEntity<Problem> entity, ServerWebExchange request) {
        log.debug("Access request path : {}", request.getRequest().getPath());
        if (entity == null) {
            return Mono.empty();
        }
    ...
    /**
     * The prod env will not show the error desc message details
     *
     * @param throwable
     * @param status
     * @param type
    ...
    public boolean isCausalChainsEnabled() {
        ...
        if (activeProfiles.contains("default") || activeProfiles.contains("local"))
            return true;
        else
            return false;
    }
